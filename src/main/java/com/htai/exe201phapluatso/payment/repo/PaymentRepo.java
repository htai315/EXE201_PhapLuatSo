...
public interface PaymentRepo extends JpaRepository<Payment, Long>, JpaSpecificationExecutor<Payment> {
        ...
        /**
         * Atomic claim webhook - prevents duplicate processing via race condition.
         * Only one thread can claim the webhook for processing.
         * 
         * @return 1 if claimed successfully, 0 if already claimed by another thread
         */
        @Modifying
        @Query("""
                        UPDATE Payment p
                        SET p.webhookProcessed = true
                        WHERE p.orderCode = :orderCode
                          AND (p.webhookProcessed = false OR p.webhookProcessed IS NULL)
                        """)
        int claimWebhook(@Param("orderCode") Long orderCode);

        /**
         * Find payments that are PAID but credit failed - for retry job.
         * These payments received money but credits haven't been added yet.
         */
        @Query("SELECT p FROM Payment p LEFT JOIN FETCH p.plan LEFT JOIN FETCH p.user WHERE p.status = 'PAID_CREDIT_FAILED' AND p.creditRetryCount < :maxRetries ORDER BY p.paidAt ASC")
        List<Payment> findPaidCreditFailedPayments(@Param("maxRetries") int maxRetries);
        ...
        @Query("SELECT p FROM Payment p WHERE p.user = :user AND p.status = '" + Payment.STATUS_CREDITED + "' ORDER BY p.createdAt DESC")
        ...
        @Query(value = "SELECT p FROM Payment p LEFT JOIN FETCH p.user LEFT JOIN FETCH p.plan", countQuery = "SELECT COUNT(p) FROM Payment p")
        org.springframework.data.domain.Page<Payment> findAllWithUser(
         ...
         * Returns: [totalPayments, successCount, failedCount, pendingCount,
         * totalRevenue]
         */
        @Query(value = """
                        SELECT
                            COUNT(*) as totalPayments,
                            SUM(CASE WHEN status = 'CREDITED' THEN 1 ELSE 0 END) as successCount,
                            SUM(CASE WHEN status = 'FAILED' THEN 1 ELSE 0 END) as failedCount,
                            SUM(CASE WHEN status = 'PENDING' THEN 1 ELSE 0 END) as pendingCount,
                            COALESCE(SUM(CASE WHEN status = 'CREDITED' THEN amount ELSE 0 END), 0) as totalRevenue
                        FROM payments
                        """, nativeQuery = true)
        ...
        @Query(value = """
                        SELECT
                            COALESCE(SUM(CASE WHEN status = 'CREDITED' THEN amount ELSE 0 END), 0) as totalRevenue,
                            COALESCE(SUM(CASE WHEN status = 'CREDITED' AND created_at >= :today THEN amount ELSE 0 END), 0) as revenueToday,
                            COALESCE(SUM(CASE WHEN status = 'CREDITED' AND created_at >= :weekStart THEN amount ELSE 0 END), 0) as revenueThisWeek,
                            COALESCE(SUM(CASE WHEN status = 'CREDITED' AND created_at >= :monthStart THEN amount ELSE 0 END), 0) as revenueThisMonth
                        FROM payments
                        """, nativeQuery = true)
        ...
        /**
         * Get revenue grouped by date for chart (avoid loading all payments into
         * memory)
        ...
        @Query(value = """
                        SELECT created_at::DATE as date,
                               ...
                               COUNT(*) as count
                        FROM payments
                        WHERE status = 'CREDITED' AND created_at BETWEEN :startDate AND :endDate
        ...
        @Query(value = """
                        ...
                        FROM payments
                        WHERE user_id IN :userIds AND status = 'CREDITED'
                        GROUP BY user_id
                        """, nativeQuery = true)
